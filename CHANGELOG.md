...
- Tap-to-type amount: on iOS, tapping the pending amount number opens a field to type an exact value; serving-size shortcuts moved to the top of the drink menu. The invisible cup-drag and the watchOS crown still work. (`feat/drink-catalog`)

### Fixed

- Cup fill now rises from the bottom instead of pouring in from the top: `WaveWithCupHeight` was missing the fill-fraction clamp its sibling `WaveWithBodyHeight` already had, so a transient over-full ratio on launch pushed the wave above the cup. (`feat/drink-catalog`)
